...
{
	GLuint VBO;
	// Note: Position is in the NDC space, so the shader will no need an projection matrix
	// this is different with SpriteRenderer
	// but the texCoords is the same
	// Note: mine, when cull_face enabled, screen is black
//	GLfloat vertices[] = {
//			-1.0f, -1.0f, 0.0f, 0.0f,
//			-1.0f, 1.0f, 0.0f, 1.0f,
//			1.0f, -1.0f, 1.0f, 0.0f,
...
//	};
	...
	//note origin code
	// NOTE: NDC space is a left-hand coordinate, so, -1,-1 is bottom-left
	// but, tex coords 's origin is also bottom-left ??
	GLfloat vertices[] = {
			// Pos        // Tex
			-1.0f, -1.0f, 0.0f, 0.0f,
			1.0f,  1.0f, 1.0f, 1.0f,
